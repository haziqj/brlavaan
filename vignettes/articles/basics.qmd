...
&= \frac{1}{2}\sum_{i=1}^n (y_i - \mu)^\top \Sigma^{-1} \cdot \frac{\partial \mu}{\partial \theta_\mu} \\
&= \frac{n}{2} (\bar y - \mu)^\top \Sigma^{-1} \cdot \frac{\partial \mu}{\partial \theta_\mu}.
\end{align}
$$
...
- $\frac{\partial\ell}{\partial \alpha} = \frac{n}{2} (\bar y - \mu)^\top \Sigma^{-1} \Lambda$.
...
# Compare
c(list(truth = tru[1:6]), lapply(fit, \(x) round(coef(x)[1:6], 3)))
