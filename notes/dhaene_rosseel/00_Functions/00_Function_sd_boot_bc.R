...
sd_boot_bc <- function(fit = NULL, 
  ...
  boot.se <- apply(u, MARGIN = 2, sd)
  boot.bias <- colMeans(u) - coef(fit)
  theta.bc <- 2*coef(fit) - colMeans(u)
  ...
  ci.norm <- c("LB" = coef(fit) - boot.bias - 1.96*boot.se,
               "UB" = coef(fit) - boot.bias + 1.96*boot.se)
  ...
  ci.basic <- c("LB" = 2*coef(fit) - apply(X = u, 
                                           MARGIN = 2, 
                                           FUN = quantile, 
                                           probs = 0.975, 
                                           type = 6),
                "UB" = 2*coef(fit) - apply(X = u, 
                                           MARGIN = 2, 
                                           FUN = quantile, 
                                           probs = 0.025, 
                                           type = 6))
  ...
  boot.t.table <- sweep(x = u, MARGIN = 2, STAT = coef(fit), FUN = "-")/w
  ...
  lav.se <-  parameterEstimates(fit)[!is.na(parameterEstimates(fit)$z), "se"]
  ci.stud <- c("LB" = coef(fit) - critical.values[2, ]*lav.se,
               "UB" = coef(fit) - critical.values[1, ]*lav.se)
  ...
  names(LBs) <- names(UBs) <- names(coef(fit))
  ...
  counts <- colMeans(sweep(u, MARGIN = 2, STATS = coef(fit), FUN = "<"))
  ...
  alpha <- 0.025
  alpha1 <- pnorm(b.term + (b.term + qnorm(alpha))/
                    (1-a.term*(b.term+qnorm(alpha))))
  alpha2 <- pnorm(b.term + (b.term + qnorm(1-alpha))/
                    (1-a.term*(b.term + qnorm(1-alpha))))
