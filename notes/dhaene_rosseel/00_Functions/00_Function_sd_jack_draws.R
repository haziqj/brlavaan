...
sd_jack_draws <- function(fit = NULL) {
  ...
  for(i in 1:nrow(est)) {
    ...
    # fit model
    if(opt.model == "growth"){
      fit.i <- try(growth(model = parameterTable(fit), 
                          data = Y, 
                          estimator = "ML", 
                          optim.attempts = 1,
                          bounds = opt.bounds,
      ...
      fit.i <- try(sem(model = parameterTable(fit), 
                       data = Y, 
                       estimator = "ML", 
                       optim.attempts = 1,
                       bounds = opt.bounds,
