...
# Need to download D&R result data from osf
here::i_am("tests/testthat/test-DRfit.R")
dr_file1 <- here::here("inst/testdata/2FSEM_est_combined_final.RData")
dr_file2 <- here::here("inst/testdata/GCM_est_combined_final.RData")
if (!file.exists(dr_file1))
  download.file("https://osf.io/cw5b7/download", destfile = dr_file1)
if (!file.exists(dr_file2))
  download.file("https://osf.io/vjq5m/download", destfile = dr_file2)
...
test_that("Growth curve model results are similar with D&R", {
  ...
  for (i in seq_len(nrow(DR_Res))) {
    ...
    expect_equal(est, estDR, tolerance = 1e-1, ignore_attr = TRUE)
  }
})
