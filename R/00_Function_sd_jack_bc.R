...
sd_jack_bc <- function(fit = NULL, jack.resamples = NULL) {
  ...
  if(sum(jack.resamples[ , "convergence"], na.rm = TRUE) == n) {
    ...
    u <- jack.resamples[ , paste0("est_", names(lavaan::coef(fit)))]
    jack.se <- sqrt((n-1)/n*colSums(sweep(u, 2, colMeans(u), FUN = "-")^2))
    jack.bias <- (n-1)*(colMeans(u)-lavaan::coef(fit))
    theta.bc <- n*lavaan::coef(fit) - (n-1)*colMeans(u)

    ci.norm <- c("LB" = lavaan::coef(fit) - jack.bias - 1.96*jack.se,
                 "UB" = lavaan::coef(fit) - jack.bias + 1.96*jack.se)

  } else {

    jack.se <- rep(NA, times = length(lavaan::coef(fit)))
    jack.bias <- rep(NA, times = length(lavaan::coef(fit)))
    theta.bc <- rep(NA, times = length(lavaan::coef(fit)))
    ci.norm <- rep(NA, times = length(lavaan::coef(fit))*2)
