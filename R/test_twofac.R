# Script to test Growth Curve Model functions

library(tidyverse)
library(lavaan)
library(tinytest)
source("R/10-sem_rbm_functions.R")
source("R/20-gen_data.R")

# Reliability == 0.8 -----------------------------------------------------------
dat <- gen_data_twofac(n = 100, rel = 0.8, dist = "Normal", lavsim = TRUE)
mod <- txt_mod_twofac()

fit_lav   <- cfa(mod, dat)
fit_ML    <- fit_sem(mod, dat, method = "ML", lavfun = "cfa")
fit_eRBM  <- fit_sem(mod, dat, method = "eRBM", lavfun = "cfa")
fit_iRBM  <- fit_sem(mod, dat, method = "iRBM", lavfun = "cfa")
fit_iRBMp <- fit_sem(mod, dat, method = "iRBMp", lavfun = "cfa")

tinytest::expect_equal(
  as.numeric(coef(fit_lav)),
  coef(fit_ML),
  check.attributes = FALSE,
  tol = 1e-5
)

tibble(
  param = names(coef(fit_lav)),
  truth = truth(dat),
  lav = as.numeric(coef(fit_lav)),
  ML = coef(fit_ML),
  eRBM = coef(fit_eRBM),
  iRBM = coef(fit_iRBM),
  iRBMp = coef(fit_iRBMp),
)

# Reliability == 0.5 -----------------------------------------------------------
dat <- gen_data_twofac(n = 1000, rel = 0.5, dist = "Non-normal")

fit_lav   <- sem(mod, dat)
fit_ML    <- fit_sem(mod, dat, method = "ML")
fit_eRBM  <- fit_sem(mod, dat, method = "eRBM")
fit_iRBM  <- fit_sem(mod, dat, method = "iRBM")
fit_iRBMp <- fit_sem(mod, dat, method = "iRBMp")
