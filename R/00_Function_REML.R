...
est.REML <- function(lme4model) {
  ...
  vc <- as.data.frame(lme4::VarCorr(lme4model))$vcov
  fe <- lme4::fixef(lme4model)
  ...
  ses <- sqrt(Matrix::diag(lme4::vcov.merMod(lme4model, full = TRUE)))
...
globalVariables("fit.REML")

## ----- HJ addition -----------------------------------------------------------
dr_reml_growth <- function(data) {
  data.long <- try(
    cbind(Subject = 1:nrow(data), data) %>%
  ...
  fit.REML <- try(
    lme4::lmer(Value ~ 1 + Day + (1 + Day | Subject),
               data = data.long, REML = TRUE),
    silent = TRUE
  )

  est.REML(fit.REML)
}
globalVariables(c(".", "Day", "Subject", "Value"))
