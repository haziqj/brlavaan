...
Loglik <- function(pars, dat, L, i = NULL) {
    ...
    ll_contr <- dmvnorm(dat, mean = Lambda %*% alpha, sigma = sigma, log = TRUE)
    if (is.null(i)) {
        sum(ll_contr)
    } else {
        ll_contr[i]
...
expect_equal(Loglik(lav_c, dat, Lambda),
             c(logLik(fit_lav))[1],
             check.attributes = FALSE)

expect_equal(numDeriv::grad(Loglik, x = lav_c, dat = dat, L = Lambda),
             fit_lav@optim$dx,
             tolerance = 1e-06)
...
expect_equal(numDeriv::grad(pen_Loglik, coef(fit_iRBM)[coef_pos], dat = dat, L = Lambda),
             rep(0, 6),
             tolerance = 1e-03)
