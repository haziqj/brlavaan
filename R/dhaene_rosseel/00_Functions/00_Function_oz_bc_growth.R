...
yr_ozenne_bcs <- function(lavobject, 
                          verbose = FALSE,
                          max.iter = 100L, 
                          tol = 1e-05,
                          return.se = FALSE) {
  ...
  L.THETA2 <- as.matrix(rowSums(L.THETA))
  G2 <- lav_matrix_duplication_ginv_pre(L.THETA2)
  ...
  G1 <- lav_matrix_duplication_ginv_pre(LxL) %*% L.PSI
  G <- cbind(G1, G2)
  if(conditional.x) {
    Sigma.k <- Sigma <- lavTech(lavobject, "implied")[[1]]$res.cov
  } else {
    Sigma.k <- Sigma <- lavTech(lavobject, "implied")[[1]]$cov
  }
  lavmodel.k <- lavmodel
  VCOV.k <- VCOV <- lavTech(lavobject, "vcov")
  Delta.mu.k <- Delta.mu <- lav_model_dmu_dtheta(lavobject@Model)
  old.k <- lav_matrix_vech(Sigma.k)
  ...
  for(k in seq_len(max.iter)) {
    ...
    out <- solve(crossprod(G)) %*% t(G) %*% lav_matrix_vech(Sigma.k)

    # reconstruct psi
    MLIST$psi <- matrix(L.PSI %*% out[1:ncol(L.PSI)], nfac, nfac)
    # reconstruct theta
    MLIST$theta <- diag(nvar) * out[length(out)]

    # updated parameter vector and lavmodel
    theta.updated <- lav_model_get_parameters(lavmodel, GLIST = MLIST,
                                              type = "free")

    # recreate lavmodel with update parameter vector
    lavmodel.k <- lav_model_set_parameters(lavmodel, x = theta.updated)
    # e. + f. update VCOV.k
    VCOV.k <- lavaan:::lav_model_vcov(lavmodel = lavmodel.k,
                                      lavsamplestats = lavsamplestats,
                                      lavoptions = lavoptions,
                                      lavdata = lavdata) * N

    # update Delta.mu (needed?)
    Delta.mu.k <- lavaan:::computeDelta(lavmodel.k)[[1]][1:nvar, ,drop = FALSE]
    
  } # iterations

  cor.se <- ifelse(test = grepl("~1", names(coef(fit))),
...
lav_model_dmu_dtheta <- function(lavmodel) {
  ...
  for(mm in seq_len(lavmodel@nmat)) {
    ...
    Delta.mu[, x.el.idx[[mm]]] <-
      lavaan:::derivative.mu.LISREL(m = mname, idx = m.el.idx[[mm]],
                                    MLIST = GLIST)
  } # mm
  Delta.mu
}
