...
yr_ozenne_bcs_growth <- function(lavobject,
                          verbose = FALSE,
                          max.iter = 100L,
                          tol = 1e-05,
                          return.se = FALSE) {
  ...
  G2 <- lavaan::lav_matrix_duplication_ginv_pre(L.THETA2)
  ...
  for(k in seq_len(max.iter)) {
    ...
    theta.updated <- lavaan::lav_model_get_parameters(lavmodel, GLIST = MLIST,
                                              type = "free")
...
lav_model_dmu_dtheta <- function(lavmodel) {
  ...
  for(mm in seq_len(lavmodel@nmat)) {
    ...
    Delta.mu[, x.el.idx[[mm]]] <-
      lavaan___derivative.mu.LISREL(m = mname, idx = m.el.idx[[mm]],
                                    MLIST = GLIST)
  } # mm
  Delta.mu
}
