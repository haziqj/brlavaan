...
The Bootstrap and Jackknife methods are resampling techniques that provide an empirical way of estimating the bias inherent in parameter estimates.
The estimate of the bias is then subtracted from the original estimate to obtain a bias-corrected estimate.
Although similar, the two methods differ in their approach to resampling.
The bootstrap involves generating numerous replicated data sets $\{y^{*}_{(b)}\}_{b=1}^B$ by sampling with replacement from the original data set, subsequently refitting the model to each bootstrap sample to obtain estimates $\hat\theta^{*}_{(b)}$.
...
On the other hand, the jackknife method involves obtaining jackknife estimates $\hat\theta_{(i)}$ by repeatedly fitting the model to $n$ subsamples of size $n-1$, systematically leaving out one observation at a time from the data set.
The jackknife bias-corrected estimator is computed as:
$$
...
Therefore, this estimation method is only relevant to the GCM model, and is not applicable to the two-factor SEM.
The GCM model @fig-pathgrowth can be expressed as a linear mixed model, where the latent intercept and slope are treated as random effects, with model equations
$$
\begin{gathered}
...
\end{gathered}
$$
Here, $y_i\in\mathbb R^p$ are the item responses as before, $X_i$ and $Z_i$ are the fixed and random design matrices, respectively, $\beta$ is the vector of fixed effects, $u_i$ is the vector of random effects, and $\epsilon_i$ is the vector of residual errors.
