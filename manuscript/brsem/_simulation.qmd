...
The bootstrap and jackknife methods are resampling techniques that provide an empirical way of estimating the bias inherent in parameter estimates, which is then subtracted from the original estimate to obtain a bias-corrected estimate.
...
$$
Although intuitive and straightforward to implement, each has their limitations.
Computing either bootstrap or jackknife estimates may be problematic in small sample sizes due to poor convergence of complex SEM models, and becomes computationally intensive for large samples.

#### REML

REML, while not an explicit bias reduction technique, is frequently employed in the estimation of variance components, particularly within the linear mixed effects framework.
As noted earlier, the latent growth curve models may be expressed as a linear mixed model, where the latent intercept and slope are treated as random effects with model equations
$$
\begin{gathered}
...
\end{gathered}
$$
Here, $y_i\in\mathbb R^p$ are the observed item responses as before, $X_i$ and $Z_i$ are the fixed and random design matrices, respectively, $\beta$ is the vector of fixed effects, $u_i$ is the vector of random effects, and $\epsilon_i$ is the vector of residual errors.
Specifically, the latent GCM in @fig-path-growth has $X_i$ and $Z_i$ both $10 \times 2$ matrices where the first column contains all 1s and the second column contains the time points 0 to 9 in increments of 1--the same for all $i$.
Further, the coefficient 2-vector $\beta$ in fact contains the latent intercepts $\alpha_1$ and $\alpha_2$, while the vector $u_i$ represents the two latent factors.
Therefore, the covariance matrix $\Sigma_u$ contains elements $\Psi_{11}$, $\Psi_{22}$, and $\Psi_{12}$.

Linear mixed models elicits a Gaussian likelihood.
...
Variance and covariance parameters estimated this way yield less biased estimates, and is particularly beneficial for data characterised by small sample sizes and complex covariance structures.
For further details, see ...

#### Ozenne et al.'s method

Explain Ozenne...  @ozenne2020small
