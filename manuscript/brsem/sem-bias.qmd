...
100, 1000\}$. The second factor is the average reliability of the
...
settings in @tbl-truth-twofac for the two-factor model, and the the
two parameters settings in @tbl-truth-growth for the latent CGM, which
...
data is attributable to measurement error, and can exacerbate finite
sample bias, particularly in the estimation of variance components,
impacting both estimation and inference performance. The third factor
is the distribution of $\eta$ and $\epsilon$, which we set to be
either "normal", "kurtosis" or "non-normal".
...
reli <- function(Lambda, B, Psi, Theta) {
    imbinv <- solve(diag(nrow(B)) - B)
    Ss <- Lambda %*% imbinv %*% Psi %*% t(imbinv) %*% t(Lambda)
    mean(diag(Ss) / diag(Ss + Theta))
}

## Two factor
Lambda <- cbind(c(1, 0.7, 0.6, 0, 0, 0), c(0, 0, 0, 1, 0.7, 0.6))
B <- cbind(c(0, 0.25), c(0, 0))
Theta_h <- diag(c(0.25, 0.1225, 0.09, 0.25, 0.1225, 0.09))
Theta_l <- diag(c(1, 0.49, 0.36, 1, 0.49, 0.36))
Psi_l <- Psi_h <- diag(c(1, 1))

c("low" = reli(Lambda, B, Psi_l, Theta_l),
  "high" = reli(Lambda, B, Psi_h, Theta_h))

## GCM
Lambda <- cbind(1, 0:9)
B <- matrix(0, 2, 2)
Psi_h <- cbind(c(550, 40), c(40, 100))
Psi_l <- cbind(c(275, 20), c(20, 50))
Theta_h <- diag(rep(500, 10))
Theta_l <- diag(rep(1300, 10))
