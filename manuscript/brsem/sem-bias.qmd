...
R(\theta) = \frac{1}{p}\sum_{j = 1}^p \frac{\Sigma^*_{jj}(\theta)}{\Sigma_{jj}(\theta)} \, .
$$ with $\Sigma^*(\theta)$ and $\Sigma(\theta)$ as in ([-@eq-marg-mom]).
For $\theta$, we consider the two parameters settings in @tbl-truth-twofac for the two-factor model, and the two parameters settings in @tbl-truth-growth for the latent GCM, which result in reliability being either about 0.5 (low) or about 0.8 (high).
...
We simulate datasets to have the covariance matrix ([-@eq-marg-mom]), as dictated by the parameters settings in @tbl-truth-twofac and @tbl-truth-growth, for the two-factor model and the latent GCM, respectively, but determine the distribution of the latent factors and the measurement errors using the `{covsim}` [@gronneberg2022covsim] R package.
...
Across the $5 \times 2 \times 2 = 20$ distinct experimental settings for sample size $n$, reliability $R(\theta)$ and distribution for each of the two-factor model and latent GCM model, we simulate 2000 independent data sets, and the corresponding model is estimated using ML estimation, explicit and implicit RBM estimation (eRBM and iRBM, respectively), the method in @ozenne2020small [Section 4], and the bootstrap and jacknife adaptations in @dhaene2022resampling.
...
For the two-factor model and the latent GCM, we require all those elements to be less than 5 and 500, respectively, reflecting differences in the scaling of observed variables.

We evaluate the methods in terms of simulation-based estimates of relative mean bias $\sum_{r=1}^{R}(\dot{\theta}^{(r)} - \theta) / R$, probability of underestimation $\sum_{r=1}^{R}\mathbb{1}(\dot{\theta}^{(r)}_j < \theta_j)/R$, and root mean squared error $\sqrt{\sum_{r=1}^{R}(\dot{\theta}^{(r)}_j -
...
We also evaluate the methods in terms of the simulation-based estimate of the conditional coverage of $95\%$ Wald-type confidence intervals at the estimates $$
...
e(\dot\theta^{(r)}) j(\dot\theta^{(r)})^{-1}$ for ML, eRBM, iRBM, and the method in @ozenne2020small \[Section 4\]. For bootstrap and jackknife, $s^{(r)}_j$ is taken to be the bootstrap and jackknife estimate of the standard error \[**IK: add a reference that corresponds to what @dhaene2022resampling** [**Section 5.3**] **refer to** \].
...
```{r}
#| label: tbl-convsucc
#| tbl-cap: 'Rate of acceptable estimates for estimation of the two-factor model using ML, eRBM, iRBM and bootstrap. Highlighted in shades of red are the scenarios which produced success rates below 50%. [**IK: We still get kurtosis here, which I manually filtered out; has `res_conv` been updated with the new simulation results? HJ: Yes. This comment can be deleted.**]'
#| html-table-processing: none
...
For the experiments we carried out, nearly all estimates from jackknife (minimal rate of acceptable estimates is $98.9\%$, as also reported in @dhaene2022resampling \[Section 6\]), and all estimates from ML and the method in @ozenne2020small [Section 4] have been acceptable \[**IK: is this correct? HJ: Yes, for ML all are 100%, Ozenne minimal rate was 99.3% at twofact n=15 non-normal data**\].
The same holds for REML estimates for the latent GCM.
...
For the latent GCM, bootstrap produced almost no acceptable estimates for $n = 15$ and $n = 20$, while eRBM produced acceptable estimates in all cases \[**IK: just quickly checking: is there any chance that we mixed the rates for eRBM and iRBM for GCM, while computing `res_conv`? HJ: Double checked this--no mixup. iRBM slightly more non-convergence compared to eRBM for gorwth models.**\].
...
The vast majority of failures where due to unsuccessful convergence of the optimization procedure we used \[**IK: say which**\].
Trying a set of candidate optimization procedures on each sample, and keeping the estimates from those that succeed, would substantially increase the rate of acceptable estimates for iRBM.
...
\[**IK: Does "results.RData" have kurtosis and non-normal results? HJ: Yes, I kept them... But we talked about not presenting the Kurtosis results.**\]
