...
by the small number of individuals who meet these specific criteria [@quezada2016explanatory].  Similarly, studies focusing on rare genetic
disorders [@figueroa2021structural], elite athletes [@hodge2009athlete;@fabbricatore2023componentbased], or unique cultural groups [@manuela2013pacific] often face strict
limitations in sample size due to the rarity of the target
population. In such contexts, the use of
...
ignored. [**HJ: YR to check references**]
...
including those that are robust to departures of the normality
assumptions.
...
\end{aligned}
$$ {#eq-marg-mom}
with $\Sigma^*(\theta) = \Lambda (I - B)^{-1} \Psi (I-B)^{-\top} \Lambda^\top$, and $A^{-\top} = (A^{-1})^\top$.
...
are near the boundary of the parameter space [@davison1997bootstrap, Sec. 5.2].
...
model $y \sim \text{N}(X\beta, \sigma^2 I)$. That marginal likelihood ends up not
...
where $V(\theta) = Z\Sigma_u Z^\top + \sigma^2 I$ is the variance-covariance
...
the `{numDeriv}` R package [@gilbert2022numderiv]. When
...
The RBM estimators have the same asymptotic distribution as the
original estimators, ensuring that standard inference and model
selection procedures apply directly. This includes procedures that are
robust to departures from the normality assumptions in
([-@eq-measurement]) and ([-@eq-structural]). For example, estimation
of standard errors for use in test statistics or confidence intervals
can proceed using robust estimators of those [**IK: add citations**].
...
Hence, the latent GCM model has $6$ parameters to be estimated.  The
corresponding path diagram is shown in @fig-path-growth. Furthermore,
the latent GCM model can be written in the equivalent linear mixed
...
We assess the robustness and performance of RBM estimation, by
considering all possible combinations of three experimental
factors. The first factor is the sample size $n \in \{15, 20, 50,
100, 1000\}$. 

The second factor is the average reliability of the items, which is
defined as
$$
R(\theta) = \frac{1}{p}\sum_{j = 1}^p \frac{\Sigma^*_{jj}(\theta)}{\Sigma_{jj}(\theta)} \, .
$$
with $\Sigma^*(\theta)$ and $\Sigma(\theta)$ as in
([-@eq-marg-mom]). For $\theta$, we consider the two parameters
settings in @tbl-truth-twofac for the two-factor model, and the 
two parameters settings in @tbl-truth-growth for the latent GCM, which
result in reliability being either about 0.5 (low) or about 0.8
(high).  Low reliability indicates that much of the variance in the
data is attributable to measurement error, and can result in finite
sample bias, particularly in the estimation of variance components,
impacting both estimation and inference performance. 
...
The third factor is the distribution of $\eta$ and $\epsilon$, which
we set to be either "normal" or "non-normal", for both the latent
factors and the measurement errors. We simulate datasets to have the
covariance matrix ([-@eq-marg-mom]), as dictated by the parameters
settings in @tbl-truth-twofac and @tbl-truth-growth, for the
two-factor model and the latent GCM, respectively, but determine the
distribution of the latent factors and the measurement errors using
the `{covsim}` [@gronneberg2022covsim] R package. The `{covsim}` R
package allows to generate multivariate data with a target covariance
matrix while controlling for the excess kurtosis and skewness of the
distribution. In the case of the normal distribution, both the
skewness and excess kurtosis are set to $0$. In the non-normal case,
skewness is set to $-2$ with excess kurtosis of $6$, producing
asymmetric, heavy-tailed distributions.  @fig-compare-dist shows the
shapes of the distributions for normal and non-normal distributions.
...
Across the $5 \times 2 \times 2 = 20$ distinct experimental settings
for sample size $n$, reliability $R(\theta)$ and distribution for each
of the two-factor model and latent GCM model, we simulate 2000 
independent data sets, and the corresponding model is estimated using
ML estimation, explicit and implicit RBM estimation (eRBM and iRBM,
respectively), the method in @ozenne2020small [Section 4], and the
bootstrap and jacknife adaptations in @dhaene2022resampling. Due to
the equivalence of the latent GCM model with a linear mixed effects
model, REML is also used to estimate the latent GCM model.

The performance of each method is first evaluated in terms of the
number of samples that result in acceptable estimates. We define
acceptable estimates are those that result i) from successful
convergence of the optimization procedure that computes them, ii) in
positive definite $\Sigma(\theta)$ at the estimates, and iii) in the
diagonal elements of $j(\theta)^{-1}$ at the estimates being within
acceptable numerical ranges. For the two-factor model and the latent
GCM, we require all those elements to be less than 5 and 500,
respectively, reflecting differences in the scaling of observed
variables.

We evaluate the methods in terms of simulation-based estimates of
relative mean bias $\sum_{r=1}^{R}(\dot{\theta}^{(r)} - \theta) / R$,
probability of underestimation
$\sum_{r=1}^{R}\mathbb{1}(\dot{\theta}^{(r)}_j < \theta_j)/R$, and root
mean squared error $\sqrt{\sum_{r=1}^{R}(\dot{\theta}^{(r)}_j -
\theta_j)^2 / R}$, where $R$ is the number of samples that resulted in
acceptable estimates for each method, and $\dot{\theta}^{(r)}_j$ is
the $j$th component of the vector of estimates from the $r$th sample
that resulted in acceptable estimates. In that way, the relative mean
bias, probability of underestimation, and root mean squared error that
we estimate are conditional on the estimator taking an acceptable
value. We also evaluate the methods in terms of the simulation-based
estimate of the conditional coverage of $95\%$ Wald-type confidence
intervals at the estimates
$$
 \frac{1}{R}\sum_{r=1}^{R}\mathbb{1}\left(\dot{\theta}^{(r)}_j - 1.96 s^{(r)}_j \leq \theta_j \leq \dot{\theta}^{(r)}_j + 1.96 s^{(r)}_j\right) \, ,
 $$
where $s_j^{(r)}$ is the estimated standard error corresponding to
$\hat\theta^{(r)}_r$. As mentioned in @sec-rbm, the RBM estimators
have the same asymptotic distribution as the ML estimator. Hence, we
can estimate standard errors using the variance-covariance matrix of
the asymptotic distribution of the ML estimator under model
mis-specification. Specifically, $s^{(r)}_j$ is taken to be the square
root of the $j$th diagonal element of $j(\dot\theta^{(r)})^{-1}
e(\dot\theta^{(r)}) j(\dot\theta^{(r)})^{-1}$ for ML, eRBM, iRBM, and
the method in @ozenne2020small [Section 4]. For bootstrap and
jackknife, $s^{(r)}_j$ is taken to be the bootstrap and jackknife
estimate of the standard error [**IK: add a reference that corresponds
to what @dhaene2022resampling [Section 5.3] refer to **]. We do not
consider alternative internals for bootstrap, like basic, studentized,
percentile and BCa intervals [see, @davison1997bootstrap, for details]
as they have been found to perform similarly to the normal
approximation interval in the experimental settings we consider here
[see @dhaene2022resampling]. For REML, which can be used to estimate the GCM model,
$s^{(r)}_j$ is obtained from the square root of the corresponding
diagonal element of the output of `vcov.merMod()` method of `{lme4}` R
package [@bates2015fitting] for fitting LMMs.
...
For the experiments we carried out, nearly all estimates from
jackknife (minimal rate of acceptable estimates is $98.9\%$, as also
reported in @dhaene2022resampling [Section 6]), and all estimates from
ML and the method in @ozenne2020small [Section 4] have been acceptable
[**IK: is this correct? HJ: Yes, for ML all are 100%, Ozenne minimal rate was 99.3% at twofact n=15 non-normal data**]. The same holds for REML estimates for the
latent GCM. For eRBM, iRBM and bootstrap, @tbl-convsucc shows the
rates of acceptable estimates for each of the two-factor and latent
GCM model, for all combinations of experimental factors we considered.

The bootstrap rates are computed based on the summaries in
@dhaene2022resampling [Table 1]. In all cases, the rates of acceptable
estimates increase as the sample size increases. For sample sizes $n
\geq 50$, all rates of acceptable estimates were above $85\%$,
reaching $100\%$ for $n = 100$ and $n = 1000$.

For the two factor model and $n = 15$ and $n = 20$, bootstrap and eRBM
results in low rates, most notably in the low reliability setting,
with the bootstrap rates being substantially lower the eRBM ones. For
the latent GCM, bootstrap produced almost no acceptable estimates for
$n = 15$ and $n = 20$, while eRBM produced acceptable estimates in all
cases [**IK: just quickly checking: is there any chance that we mixed
the rates for eRBM and iRBM for GCM, while computing `res_conv`? HJ: Double checked this--no mixup. iRBM slightly more non-convergence compared to eRBM for gorwth models.**].

Since the ML estimates have been acceptable in all samples, the
non-acceptable eRBM estimates are due to the post-hoc correction
resulting non-positive definite estimate of $\Sigma(\theta)$ or
excessively large estimated standard errors. As a post-hoc correction,
bootstrap can also result in non-positive definite estimates of
$\Sigma(\theta)$ or excessively large estimated standard errors, but,
in addition, its rates are impacted by non-acceptable ML estimates
from bootstrap samples. Hence, caution should be exercised when using
eRBM or bootstrap in very small sample scenarios.

The rates of acceptable estimates from iRBM are at least $80%$ in all
cases. The vast majority of failures where due to unsuccessful
convergence of the optimization procedure we used [**IK: say
which**]. Trying a set of candidate optimization procedures on each
sample, and keeping the estimates from those that succeed, would
substantially increase the rate of acceptable estimates for iRBM.
