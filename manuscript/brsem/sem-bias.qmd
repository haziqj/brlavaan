...
of the two-factor model and latent GCM model, we simulate 1000 [**IK: 2000?***]
...
number of samples that result in acceptable estimates. We define acceptable
estimates are those that result i) from successful convergence of the
optimization procedure that computes them, ii) in positive definite $\Sigma(\theta)$ at
the estimates, and iii) in all estimated standard errors [**IK: are
these based on just the hessian of the objective? Hessian of the
objective is what we should use here.**] for the estimators being
within acceptable numerical ranges. For the two-factor model and the
latent GCM, we require all estimated standard errors to be less than
5 and 500, respectively, reflecting differences in the scaling of
observed variables.
...
[**IK: is this correct?**]. The same holds for REML estimates for the
...
the rates for eRBM and iRBM for GCM, while computing `res_conv`?**].
...
[**IK: Does "results.RData" have kurtosis and non-normal results?**]
```{r}
...
[**IK: Should `plot_df` ve `plot_df50` below?**]
