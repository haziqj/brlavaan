---
title: 'Simulation Results for Two-Factor Model'
format: 
  html: 
...
library(gt)
library(glue)
here::i_am("experiments/results/twofac.qmd")
...
res_filtered <-
  ...
  # filter(converged) |>
  filter(!is.na(se), abs(se) < 10, abs(bias) < 3.8, converged) 
```

## Estimation success rates

```{r}
#| label: tbl-convsucc-twofac
#| tbl-cap: 'Success rates for estimation of the two-factor model for the ML, eBR, and iBR methods.'
...
#| fig-cap: 'Centred distributions of the estimates for the two-factor model. Extreme estimate values are trimmed. The number of estimates used for the calculation of summaries is shown in the right panel.'
#| fig-height: 7.5
#| fig-width: 6
#| out-width: 100%
plot_df <-
  res |>
  filter(sim <= 1000) |>
  filter(dist == "Normal", rel == "Rel = 0.8", param %in% twofacpars) |>
  filter(abs(se) < 10, abs(bias) < 3.8, converged) |>
  mutate(
    param = factor(param, levels = rev(twofacpars)),
...
truth <-
  ...
  mutate(truth = c(truth_twofac(0.8)[twofacpars], truth_twofac(0.5)[twofacpars])) |>
...
plot_bias_twofac <- function(FUN = mean, YLIM = c(-0.4, 0.3)) {
  bind_rows(
    res,
    res_dr
  ) |>
  filter(param %in% twofacpars, dist != "Kurtosis") |>
  ...
  mutate(
    ...
    param = factor(param, levels = twofacpars, labels = c(
      expression(theta["1,1"]),
      expression(Psi["1,1"]),
      expression(Psi["2,2"]),
      expression(beta),
...
plot_bias_twofac(mean, c(-0.25, 0.25))
```
...
bind_rows(
  ...
  filter(param %in% twofacpars, dist != "Kurtosis") |>
  # filter(!method %in% c("eRBM") | abs(bias) < 10) |>
  summarise(
    rmse = sqrt(mean(bias ^ 2, na.rm = TRUE, trim = 0.05)),
    .by = c(dist:param)
  ) |>
...
tab_bias_twofac <- function(rel) {
  ...
  ress |>
    ...
    # filter(converged) |>
    filter(dist != "Kurtosis", param %in% twofacpars, i) |>
    # filter(!method %in% c("eRBM") | abs(relbias) < 1) |>
    summarise(
...
bind_rows(res, res_dr) |>
  filter(dist != "Kurtosis", method != "REML", param %in% twofacpars) |>
  # filter(!method %in% c("eRBM") | abs(relbias) < 1) |>
  summarise(
    covr = mean(covered, na.rm = TRUE),
    .by = c(dist:method, param)
